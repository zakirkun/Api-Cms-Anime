...
use App\Helpers\V1\Adfly\AuthType ;
...
class libAdfly {
	...
	public function __construct($userId, $publicKey, $secretKey) {
		$this->connection = httpAdfly::connect(self::BASE_HOST, 443, httpAdfly::HTTPS);
	...
	public function getGroups($page=1) {
        $response = json_decode($this->connection->doGet('v1/urlGroups',$this->getParams(array('_page' => $page), AuthType::HMAC)),1);
	...
	public function getAccountDetails() {
        $response = json_decode($this->connection->doGet('v1/account',$this->getParams(array(), AuthType::HMAC)), 1);
		return json_encode($response);
	}
	...
	public function getUrls($page=1, $q=null) {
		...
		$response = json_decode($this->connection->doGet('v1/urls',$this->getParams($params, AuthType::HMAC)),1);
		return json_encode($response);
	}
	...
	public function updateUrl($id, $groupId=false, $url=false, $advertType=false, $title=false, $fbDescription=false, $fbImage=false) {
		$params = array();
	...
	public function deleteUrl($id) {
        $response = json_decode($this->connection->doDelete('v1/urls/' . $id,$this->getParams(array(), AuthType::HMAC)),1);
		return json_encode($response);
    }
